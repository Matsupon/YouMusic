...
const DashboardSongs = () => {
  ...
  const [{ allSongs, isSongPlaying, song: currentSongIndex }, dispatch] = useStateValue();
  ...
  const addSongToContext = (songIndex) => {
    const clickedSong = filteredSongs ? filteredSongs[songIndex] : allSongs[songIndex];
  
    if (selectedSong !== null && selectedSong._id === clickedSong._id) {
      ...
      const actualSongIndex = allSongs.findIndex((song) => song._id === clickedSong._id);
    ...
    <div className="w-full p-4 flex items-center justify-center flex-col">
      <div className="w-full flex justify-center items-center gap-24">
        <NavLink
          ...
          className="flex items-center px-4 py-3 border rounded-md bg-ioAdd hover:border-gray-400 hover:shadow-md cursor-pointer"
        >
          <IoAdd />
        </NavLink>
        <input
          ...
          className={`w-52 px-4 py-2 border ${
            isFocus ? "border-gray-500 shadow-md" : "bg-songContainer"
...
export const SongContainer = ({ data, addSongToContext }) => {
  return (
      ...
      {data &&
        data.map((song, i) => (
          <SongCard
            ...
            data={song}
            index={i}
            addSongToContext={() => addSongToContext(i)}
          />
        ))}
    </div>
  );
...
export const SongCard = ({ data, index, addSongToContext }) => {
